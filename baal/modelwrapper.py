...
from baal.utils.metrics import Loss, Accuracy
...
class ModelWrapper:
    ...
    def __init__(self, model, criterion, replicate_in_memory=True):
        ...
        self.metrics = dict()
        self.add_metric("accuracy", lambda: Accuracy())
    ...
    def train_on_dataset(
        ...
        for _ in range(epoch):
            ...
            history.append(self.metrics["train_accuracy"].value)

        optimizer.zero_grad()  # Assert that the gradient is flushed.
        log.info("Training complete", train_accuracy=self.metrics["train_accuracy"].value)
        return history
    ...
    def test_on_dataset(
        ...
        log.info("Evaluation complete", test_accuracy=self.metrics["test_accuracy"].value)
        return self.metrics["test_accuracy"].value
